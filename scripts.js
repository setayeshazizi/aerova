...
function prevTestimonialNew() {
  currentTestimonialNew =
    (currentTestimonialNew - 1 + totalTestimonialSlides) %
    totalTestimonialSlides;
  updateTestimonialNew();
}

function goToTestimonialNew(index) {
  currentTestimonialNew = Math.max(
    0,
    Math.min(index, totalTestimonialSlides - 1),
  );
  updateTestimonialNew();
}
...
function bindRegisterForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (termsCheck && !termsCheck.checked) {
      ...
      const checkboxContainer = termsCheck.closest(".checkbox-container");
      ...
      // Add shake animation
      const formOptions = termsCheck.closest(".form-options");
      if (formOptions) {
        formOptions.style.animation = "shake 0.5s ease";
        setTimeout(() => {
          formOptions.style.animation = "";
        }, 500);
      }

      return; // Stop here - don't continue with other validations
    } else {
...
function initFlightStatus() {
  ...
  body.innerHTML = flights
    .map(([n, d, time, gate, status, style]) => {
      return `<tr>
